DROP TABLE IF EXISTS 会員,図書,所蔵品,貸出可能,貸出,返却,予約,予約取消,取置,貸出と会員;
...
DROP SCHEMA IF EXISTS 所蔵品 CASCADE;
CREATE SCHEMA 所蔵品;
...
CREATE TABLE 所蔵品.貸出可能
(
  所蔵品番号 VARCHAR(40) PRIMARY KEY REFERENCES 所蔵品.所蔵品,
  登録日時 TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
);

CREATE TABLE 所蔵品.貸出中
(
  所蔵品番号 VARCHAR(40) NOT NULL REFERENCES 所蔵品.所蔵品,
  登録日時 TIMESTAMP   NOT NULL DEFAULT CURRENT_TIMESTAMP
);

CREATE TABLE 所蔵品.取置中
(
  所蔵品番号 VARCHAR(40) PRIMARY KEY REFERENCES 所蔵品.所蔵品,
  登録日時 TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
);
