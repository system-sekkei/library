...
import library.application.coordinator.returnbook.ReturnBookResult;
...
public class ReturnBookRegisterController {
    ...
    String register(@Validated @ModelAttribute("returnBookForm") ReturnBookForm returnBookForm, BindingResult result, RedirectAttributes attributes) {
        ...
        ReturnBookResult returnBookResult = returnBookCoordinator.returnBook(returnBookForm.bookCollectionCode, returnBookForm.returnDate);

        if (returnBookResult.hasError()) {
            result.addError(new ObjectError("error", returnBookResult.message()));
            return "returnbook/register/form";
        }
